...
- 工作模式：视频拼接 / 音频拼接，可在「参数设置 · 拼接」右上角切换
- 复制模式（默认）：`concat demuxer + -c copy`，无质量损失，要求各段编码参数一致
- 兼容模式（视频）：重新编码为 H.265 + AAC 后再拼接，不同编码也能拼
- 兼容模式（音频）：各段转码为 WAV 后拼接，兼容不同采样率 / 声道
...
| --- | --- |
| `FFCraft\` | 便携目录，拷走即用 |
| `FFCraft_v1.7.1_portable.zip` | 便携版压缩包，解压后双击 `FFCraft.exe` 即可 |
